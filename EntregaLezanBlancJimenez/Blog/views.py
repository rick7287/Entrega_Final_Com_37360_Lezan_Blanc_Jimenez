...
def pagina_detalle(request, pagina_id):
    ...
    try:
        ...
        pagina = Pagina.objects.filter(id=pagina_id)
        ...
        if pagina.exists():
            ...
            #mensaje= f'entro al if'
            #return render(request, "Blog/confirmacion.html", {'mensaje':mensaje}) 
            pagina = Pagina.objects.get(id=pagina_id)
            return render(request, "Blog/pagina_detalle.html", {"pagina":pagina})

        else:
            mensaje = f"No hay publicaciones con ese id"
            return render(request, "Blog/confirmacion.html", {'mensaje':mensaje}) 

    except:
        mensaje= f"El id ingresado no es valido, debe ser un numero entero"
        return render(request, "Blog/confirmacion.html", {'mensaje':mensaje})
...
def pagina_borrar(request, pagina_id):            #Cuando mando llamar la vista, le paso un parametro, en este caso el id de una pagina
    ...
    mensaje= f'La pagina {pagina.titulo} fue eliminada'
